...
//ALLEGRO_TIMER* ice_timer = al_create_timer(1.0 / 60.0);
...
const std::vector<int> PlayScene::power = { ALLEGRO_KEY_L, ALLEGRO_KEY_L, ALLEGRO_KEY_R };
...
void PlayScene::Initialize() {
    ...
    //AddNewObject(PowerGroup = new Group());
	...
	// Preload Lose Scene
	deathBGMInstance = Engine::Resources::GetInstance().GetSampleInstance("WOW.ogg");
...
void PlayScene::Update(float deltaTime) {
    ...
    // Win
    if (DefenseGroup->GetObjects().empty()||flag == true) {
    ...
    if (armyEmpty) {
        ...
        //delete UIGroup;
        Engine::GameEngine::GetInstance().ChangeScene("lose");
        AudioHelper::PlaySample("play.ogg", true, 0.0);
    }
	...
	if (button & 1) {
		...
		if (!CheckOccupied(x, y) || preview->id == 3) {
            ...
            //else if (preview->id == 4) {
                //PowerGroup->AddNewObject(preview);
            //}
            else {
                ArmyGroup->AddNewObject(preview);
            }
            ...
            if (remainId != -1) {
                if (remainId == 0) {
                    ...
                    preview = new SofaArmy(0, 0);
                }
                else if (remainId == 3) {
                    preview = new Ice_Spell(0, 0);
                }
                else if (remainId == 4) {
                    preview = new PowerArmy(0, 0);
                }
                preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_TAB) {
        ...
        keyStrokes003.push_back(keyCode);
		if (keyStrokes.size() > code.size())
			keyStrokes.pop_front();
        if (keyStrokes003.size() > power.size())
            keyStrokes003.pop_front();

        if (keyStrokes003.size() == power.size()) {
            int i = 0;
            for (int i = 0; i < 3; i++) {
                if (keyStrokes003.front() == power[i]) {
                    keyStrokes003.pop_front();
                    if (i == 2) {
                        //if (flagg == 0) {
                        ConstructButton(4,ArmyImage[4]);
                        AudioHelper::PlayAudio("flash.wav");
                        //UIBtnClicked(4);
                           //flagg = 1;
                        //}
...
void PlayScene::ReadMap() {
	...
	while (fin >> c) {
        ...
        case 't': mapData.push_back(TILE_TRAP); break;
		case '\n':
		case '\r':
		...
		for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_FLOOR:
                    ...
                    break;
                case TILE_ENEMY2:
                    DefenseGroup->AddNewObject(new Enemy2Defense(j * BlockSize - 2 + BlockSize - 2 / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_TRAP:
                    TrapGroup->AddNewObject(new TrapDefense(j * BlockSize + BlockSize - 2 / 2, i * BlockSize + BlockSize / 2));
                    break;
                default:
                    throw std::ios_base::failure("Map data is corrupted.");
            }
